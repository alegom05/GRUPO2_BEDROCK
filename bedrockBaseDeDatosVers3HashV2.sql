...
CREATE TABLE IF NOT EXISTS `basededatos3`.`evento` (
  ...
  INDEX `fk_Evento_Usuario1_idx` (`idUsuario` ASC) VISIBLE,  -- Nuevo índice
  CONSTRAINT `fk_Evento_Profesor1`
    FOREIGN KEY (`idProfesor`)
    REFERENCES `basededatos3`.`profesor` (`idProfesor`),
  CONSTRAINT `fk_Evento_Usuario1`  -- Nueva restricción
    FOREIGN KEY (`idUsuario`)
    REFERENCES `basededatos3`.`usuario` (`idUsuario`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8;
...
('Incendio en edificio', 'Incendio en un edificio de departamentos en la Calle 3', 'Calle 3, San Miguel-Lima', 'Referencia 3, San Miguel-Lima', '934567890', 1, 1, 3, 'Alto', 'Ambulancia', 'canino', 'Se evacuaron los residentes y se está controlando el fuego', NULL, 'Policía en camino', 'En proceso', 'OT', NULL, 0),
